...
async def video_endpoint(req: Request, videoId: str):
    video = queue.db.search_entry_by_id(videoId)
    return range_requests_response(
        req, file_path=video.get("video"), content_type="video/mp4"
    )

@app.get("/download/{videoId}")
async def download(videoId: str):
    queue.enqueue(videoId)
    return {"message": f"Downloading {videoId}"}

@app.get("/notifications")
async def get_notifications():
    notifications = queue.get_notifications().copy()
    queue.clear_notifications()
